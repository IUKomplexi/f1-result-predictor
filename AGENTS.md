...
- `f1web/` — `app.py` (`create_app`, JSON API + built-SPA host), `jobs.py` (`JobManager`, threading worker + single-job queue + `reports/jobs/*.json` history; history is **reloaded at startup** — jobs left queued/running by a dead process become `interrupted`; queued jobs can be cancelled via `POST /api/jobs/{id}/cancel`, running jobs are atomic) and Preact SPA in `f1web/ui/` (Vite + TS; `@preact/preset-vite` + `preact/compat`); tabs: Status, Race, Race History, Data, Train, Backtest (also hosts the **Calibration panel** — calibrate job + `reports/calibration.json` report view), Settings — deep-linkable via URL hashes (`#/race`, `#/backtest`, …; refresh/back-button safe). The Race tab is the per-race control surface on `POST /api/predict`: model picker, interactive qualifying-grid editor, write-report, refresh (feature and hyperparameter overrides stay at training time — Train tab, or Backtest's walk-forward retraining). Tab components share the `TabProps` contract (`f1web/ui/src/types.ts`): `onNavigate(tabId, state?)` for cross-tab jumps (e.g. Race History → a specific race), `navState` payload, and `setNavigateGuard` so a tab can veto navigation (Settings' unsaved-changes dialog). The SPA is organized as one folder per tab under `src/pages/` (entry component = `<Tab>.tsx`) plus shared building blocks: `src/components/{ui,jobs,controls}` (visual primitives / job widgets / form controls), `src/hooks/`, `src/lib/` (see `f1web/ui/README.md` for the full map). Shared UI: `components/jobs/JobRunner`, `components/jobs/JobsWidget` (dialog-grade panel: focus management, Escape, outside-click, `aria-live` status announcements, queued-job Cancel buttons; header shows a running-job pill), `components/ui/ConfirmDialog`, `components/controls/RefreshToggle`, `components/ui/Chart`. The Train job also calibrates. UI checks: `npm run build` (`tsc -b && vite build`), `npm run lint` (oxlint) and `npm test` (vitest + @testing-library/preact, offline jsdom suites for the grid editor, model picker, calibration report and prediction payload builder) in `f1web/ui` — all three run in CI.
...
- The model's HGB hyperparameters are config-driven: train reads `[model.params]` (defaults in `f1core/config.py` `DEFAULTS`); the web Train tab can override them **per run** (ephemeral `params` job-payload key, validated against `MODEL_PARAM_KEYS` in `POST /api/jobs` and forwarded by `_train_handler`), so the Settings tab locks the `[model.params]` section — the deployed model keeps the params it was trained with (recorded per checkpoint in `data/model/index.json`). A params change silently requires retrain; feature-list edits change the checkpoint fingerprint (existing behavior).
