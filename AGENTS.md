...
- `f1web/` — `app.py` (`create_app`, JSON API + built-SPA host), `jobs.py` (`JobManager`, threading worker + single-job queue + `reports/jobs/*.json` history) and React SPA in `f1web/ui/` (Vite + TS); tabs: Race, Race History, Data, Train, Search, Backtest, Calibration, Specific Race, Settings, Season. Pipeline steps each run from their own tab via the shared `ui/JobRunner`. No UI *unit* suite (known gap; `npm run build`/`npm run lint` are the UI checks)
