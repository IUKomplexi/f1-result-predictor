...
Backtest's walk-forward retraining). Edits stay local until
**Apply changes**; override predictions are cached on disk per model + grid,
so repeats are instant and different models/grids never share a cached
payload.
