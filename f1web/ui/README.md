...
(`/api/*`) served by the Flask backend in `f1web/app.py`.

## Development

With the Flask backend running on port 8080:
...
Flask serves the built SPA at `/dashboard` (see `f1web/app.py`), so after
`npm run build` just start the backend:

```sh
f1-web --port 8080   # http://127.0.0.1:8080/dashboard
```
...
- `src/App.tsx` — app shell; dashboard views land in the next phase.
